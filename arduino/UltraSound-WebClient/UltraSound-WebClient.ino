...
#define WEBPAGE      "/test/Tail-Log-Receive-Data.php"//"/login/data.php"//"lab3_bumpweb/bumpmemo.php?bumplength=2.567"//"/test/Tail-Log-Receive-Data.php"//"/testwifi/index.html"
...
//  Adafruit_CC3000_Client www = cc3000.connectTCP(ip, 80);
  if (www.connected()) {
    ...
    int val_int;
    int val_fra;
    val_int = (int) cm;   // compute the integer part of the float 
    val_fra = (int) ((cm - (float)val_int) * 1000);   // compute 3 decimal places (and convert it to int)
    snprintf (str_data, sizeof(str_data), "%d.%d", val_int, val_fra); //
    Serial.print(str_data);
    Serial.println();
    //snprintf(str_data, sizeof str_data, "%.2f", cm);
    strcat(str,str_data);
    www.fastrprint(str);
    www.fastrprint(F(" HTTP/1.1\r\n"));
    www.fastrprint(F("Host: ")); www.fastrprint(WEBSITE); www.fastrprint(F("\r\n"));
